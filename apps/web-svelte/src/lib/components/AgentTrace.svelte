...
<div class="flex flex-col gap-3">
  {#if trace.length === 0 && !streaming}
    <p class="text-xs text-text-muted">Sin razonamiento aún. Envía un mensaje para ver los pasos del agente.</p>
  {/if}

  {#each trace as step (step.id)}
    <div class="flex flex-col gap-1 rounded-xl border border-bg-elevated bg-bg-card px-3 py-2">
      <div class="flex items-center justify-between">
        <span class="rounded-md border px-2 py-0.5 font-mono text-[10px] font-medium {NODE_STYLES[step.node] ?? 'border-bg-elevated bg-bg-elevated text-text-secondary'}">
  ...
  {#if streaming && (currentNode || currentAgent)}
    <div class="flex items-center gap-2 rounded-xl border border-accent-blue/30 bg-accent-blue/10 px-3 py-2">
      <span class="thinking-dot inline-block h-1.5 w-1.5 rounded-full bg-accent-blue"></span>
      <span class="thinking-dot inline-block h-1.5 w-1.5 rounded-full bg-accent-blue"></span>
      <span class="thinking-dot inline-block h-1.5 w-1.5 rounded-full bg-accent-blue"></span>
