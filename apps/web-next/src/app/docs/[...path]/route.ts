...
// Next runs from apps/web-next; the docs/ folder is two levels up.
...
export async function GET(
  ...
  if (!target.startsWith(DOCS_ROOT + sep) && target !== DOCS_ROOT) {
    return new Response("outside docs root", { status: 403 });
  }
  ...
  if (ext === ".md" || ext === ".markdown") {
    ...
    const body = (await marked.parse(raw, { gfm: true, breaks: false })) as string;
    return new Response(HTML_FRAME(title, body), {
