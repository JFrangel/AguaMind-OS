...
export default defineEventHandler(async (event) => {
  ...
  // Nuxt's catch-all stores the wildcard segments under the param key
  // matching the bracket name. The remaining path is in `params.path`.
  ...
  if (!existsSync(resolved)) {
    ...
    return `not found: ${requested}`;
  }

  const ext = extname(resolved).toLowerCase();

  if (BINARY_EXTS.has(ext)) {
    setResponseHeaders(event, { "Content-Type": MIME[ext] ?? "application/octet-stream" });
  ...
  if (ext === ".md" || ext === ".markdown") {
    ...
    const body = (await marked.parse(text, { gfm: true, breaks: false })) as string;
    setResponseHeaders(event, { "Content-Type": "text/html; charset=utf-8" });
    return HTML_FRAME(title, body);
  }
