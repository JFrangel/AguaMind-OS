# AguaMind OS - Como se ve cada capa implementada (vista visual)
...
- **AguaMind se conecta:** instrumentando cada filtro con sensores de presion diferencial (detecta saturacion antes del retrolavado), cada motobomba con vibracion (predice falla), tanque cloracion con turbidez post-filtrado.
...
- **AguaMind se conecta:** modela este layout en el SVG del dashboard `/agua` -> "Mapa del Campus", actualiza el flujo en tiempo real con datos de cada sensor.
