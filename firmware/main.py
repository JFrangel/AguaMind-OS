"""
AguaMind OS — Firmware ESP32 (MicroPython)
...
from mqtt_client import AguaMindMQTT
...
def main():
    print("=" * 50)
    print(f"AguaMind OS · ESP32 Firmware v{cfg.FIRMWARE_VERSION}")
    print(f"Node: {cfg.NODE_ID} @ {cfg.NODE_LOCATION}")
    print("=" * 50)

    # Inicializar interfaz local
    display = LocalDisplay()
    display.show_message("AguaMind OS", "Iniciando...", "v" + cfg.FIRMWARE_VERSION)
    ...
    display.show_message("AguaMind OS", "Operacional", "Loop activo")
    ...
    mqtt.disconnect()
    display.show_message("AguaMind OS", "Detenido", "")
    display.set_status("info")
